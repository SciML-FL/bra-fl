...
class Server:
    ...
    def fit_round(self, server_round: int):
        ...
        # Perform post training callback to clients.
...
def post_training(executor, client_instructions, results, failures):
    """Refine parameters concurrently on all selected clients."""
    submitted_fs = {
        executor.submit(callback_client, client, results, failures) for client, _, _, _ in client_instructions
    }
    finished_fs, _ = concurrent.futures.wait(
        fs=submitted_fs,
        timeout=None,  # Handled in the respective communication stack
...
def callback_client(client, results, failures):
    """Refine parameters on a single client."""
    fit_res = client.post_training_callback(results, failures) #, group_id=group_id)
    return client, fit_res
